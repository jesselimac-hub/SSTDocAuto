...
// =========================================================================
const handleGerarDocumento = async (req, res) => {
  try {
    const empresaId = getEmpresaId(req);
    ...
    if (!fs.existsSync(templatePath)) {
      ...
      try { fs.writeFileSync(templatePath, defaultBuf); } catch (_) {}
    }
...
async function startServer() {
  if (process.env.NODE_ENV !== 'production' && !process.env.VERCEL) {
    ...
    app.listen(PORT, '0.0.0.0', () => {
      console.log(`[SST DocAuto Backend] Servidor de produção rodando na porta ${PORT}`);
    });
  }
}
